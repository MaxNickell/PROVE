...
Handles scene_attribute subquestions using LLM decomposition + Qwen binary verification.
Follows the established subquestion analysis pattern used by attribute and relationship extractors.
"""
...
class SceneAttributeProcessor:
    ...
    def process_scene_attribute_subquestions(
        ...
        """
        Process scene_attribute subquestions using proper subquestion decomposition.
    ...
    def _analyze_scene_subquestions_for_candidates(
        ...
        for subquestion in scene_subquestions:
            if subquestion.subquestion_type != "scene_attribute":
                continue

            # Analyze this subquestion to determine required scene attributes
            candidates = self._analyze_single_subquestion_for_scene_attributes(
                llm_client, subquestion, images, image_contexts
            )

            # Add subquestion reference to candidates
            for candidate in candidates:
                candidate.required_for_subquestions = [subquestion.question]

            all_candidates.extend(candidates)

        return all_candidates

    def _analyze_single_subquestion_for_scene_attributes(
        self,
        llm_client,
        ...
        """
        Analyze a single scene subquestion to determine required scene attribute verifications.

        Args:
            llm_client: LLM client
            subquestion: Scene attribute subquestion to analyze
            images: ImageData structure
            image_contexts: Optional dict mapping image_id to caption text for processing

        Returns:
            List[SceneAttributeCandidate]: Required scene attribute verifications for this subquestion
        """
        try:
            ...
            prompt = f"""Analyze this scene attribute subquestion to determine what atomic scene attributes need verification.
...
Task: Break this subquestion into atomic scene attribute verifications that can be answered with binary Yes/No questions.
...
Answer:"""
            ...
            print(f"Warning: Failed to analyze scene subquestion '{subquestion.question}': {e}")
            return []
...
Answer:"""
            ...
            result = SceneAttributeResult(
                ...
                subquestion=candidate.required_for_subquestions[0] if candidate.required_for_subquestions else ""
            )
